...
def save_scores_csv(scores_acc, scores_kappa, scores_matthews_corrcoef, chunk_size, s_name, directory_to_save):
    ...
    range = np.arange(chunk_size, (len(balanced_acc) + 1) * chunk_size, chunk_size)
...
def read_and_run(stream_name, chunk_size, prep_methods, prep_methods_names, neurons=100, smoke_param=1,
    ...
    balanced_acc, kappa, matthews_corrcoef = streamLearner.get_scores()
    save_scores_csv(balanced_acc, kappa, matthews_corrcoef, chunk_size, stream_name, directory_to_save)
...
def start(m_methods, m_methods_names, m_streams_names):
    m_chunks_sizes = [200, 400, 800, 1000, 1500, 2000, 4000, 8000, 10000, 15000, 20000]
